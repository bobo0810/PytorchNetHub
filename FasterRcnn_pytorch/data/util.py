...
def read_image(path, dtype=np.float32, color=True):
    ...
    order of the channels is RGB.
       该功能从给定文件读取图像。 图像是CHW格式和值的范围是[0，255]。 
       如果color = True，那么通道的顺序是RGB。

    Args:
        path (str): A path of image file.图像文件的路径
        dtype: The type of array. The default value is :obj:`~numpy.float32`.
        color (bool): This option determines the number of channels.
            If :obj:`True`, the number of channels is three. In this case,
            the order of the channels is RGB. This is the default behaviour.
            If :obj:`False`, this function returns a grayscale image.
            True:在这种情况下，频道的顺序是rgb。(默认)
            False:此函数返回灰度图像
...
def resize_bbox(bbox, in_size, out_size):
    ...
      预计bboxes将被打包成二维张量形状：（R，4）
      其中：数学：`R`是图像中的边界框的个数。
      第二个轴表示属性边界框。 （y_ {min}，x_ {min}，y_ {max}，x_ {max}）`，
     其中四个属性是左上角的坐标和右下角的顶点。
...
     其中四个属性是左上角的坐标和右下角的顶点。
    ...
    Args:
        ...
        size (tuple): A tuple of length 2. The height and the width
            of the image before resized.高度和宽度
        y_flip (bool): Flip bounding box according to a vertical flip of
            an image.根据垂直翻转的方式翻转边界框
        x_flip (bool): Flip bounding box according to a horizontal flip of
            an image.根据水平翻转来翻转bboxes
...
def crop_bbox(
    ...
    此方法主要与图像裁剪一起使用。
    这个方法转换了边界框的坐标，类似于本文中的translate_bbox方法。
    此外，此功能会截断边框以适应裁剪区域。
    如果边界框不与裁剪区域重叠，此边界框将被删除。
...
def translate_bbox(bbox, y_offset=0, x_offset=0):
    ...
    此方法主要与图像转换一起使用，例如填充和裁剪，从而转换图像的左上角坐标。
